...
async function guardarEnFirebase(key, data) {
    ...
    try {
        await setDoc(doc(db, "estucontaDB", key), { payload: JSON.stringify(data) });
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- 0. SINCRONIZACIÓN INICIAL CON FIREBASE ---
    async function sincronizarDesdeFirebase() {
        ...
        for (const key of colecciones) {
            try {
                const docSnap = await getDoc(doc(db, "estucontaDB", key));
                if (docSnap.exists()) {
                    localStorage.setItem(key, docSnap.data().payload);
                }
            } catch (error) {
